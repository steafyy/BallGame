...
void checkColision(cBall* ball);
void draw_cball(cBall* ball);
void erase_cball(cBall* ball);
void move_cball(cBall* ball, uint16_t lcd_width, uint16_t lcd_height);
...
//DEFINITION OF THE FRAME BUFF ADDRESSES
...
#define FRAME_BUF_2_ADDRESS \
    0xD0100000                             ///< start address of second frame buffer's pixels (no overlap
                                           ///< with the first one)
//DISPLAYP HISICAL ADDRESS
...
void lcdSimpleTest() {
	if (initializeLCD() != LCD_OK) {
	    ...
	    fillWithColorLCD(FRAME_BUF_2, LCD_COLOR_BLACK);
	    setActiveFrameBufLCD(FRAME_BUF_2);

	    cTarget * ptrTargetArray = NULL;
	    uint8_t control1;
	    uint8_t control2;

	    for(int i = 0; i<5; i++){
	    	if(i==0){
	    		targetArray[i]=*newTarget(generate_random_position(10,225),generate_random_position(40,170),20,true);
	    	}else{
	    		control1 = 0;
	    		while(control1 == 0){
	    			targetArray[i]=*newTarget(generate_random_position(10,225),generate_random_position(40,170),20,true);
	    ...
	    for(int i = 0; i<5; i++)
	    	targetArray[i].drawFilledSquareLCD(LCD_WIDTH,LCD_HEIGHT,FRAME_BUF_1, FRAME_BUF_2,
	    			FRAME_BUF_1_ADDRESS, FRAME_BUF_2_ADDRESS, FRAME_BUF_2,LCD_COLOR_RED, &targetArray[i]);
...
void takeLife(){
	LIVES--;
	if(LIVES==0){
		BSP_LCD_DisplayStringAt(20,100,"GAME OVER",CENTER_MODE);
		//GAME OVER INTERRUPT IDK
	}
}
		...
		if(ball->base.xCoord >= (targetArray[i].base.xCoord - targetArray[i].size/2-ball->rad) &&
		   ball->base.xCoord <= (targetArray[i].base.xCoord + targetArray[i].size/2+ball->rad)){
			if(ball->base.yCoord == (targetArray[i].base.yCoord - targetArray[i].size/2 - ball->rad)){
				targetArray[i].drawFilledSquareLCD(LCD_WIDTH,LCD_HEIGHT,FRAME_BUF_1, FRAME_BUF_2,
					    			FRAME_BUF_1_ADDRESS, FRAME_BUF_2_ADDRESS, FRAME_BUF_2,LCD_COLOR_BLACK, &targetArray[i]);
				targetArray[i].isActive=false;
				ball->ySpeed *= -1; //bounce from top:
			}
			if(ball->base.yCoord == (targetArray[i].base.yCoord + targetArray[i].size/2 + ball->rad)){
				targetArray[i].drawFilledSquareLCD(LCD_WIDTH,LCD_HEIGHT,FRAME_BUF_1, FRAME_BUF_2,
									    			FRAME_BUF_1_ADDRESS, FRAME_BUF_2_ADDRESS, FRAME_BUF_2,LCD_COLOR_BLACK, &targetArray[i]);
				targetArray[i].isActive=false;
				ball->ySpeed *= -1; //bounce from bottom:
			}
		}
		   ...
		   ball->base.yCoord <= (targetArray[i].base.yCoord + targetArray[i].size/2+ball->rad)){
				if(ball->base.xCoord == (targetArray[i].base.xCoord - targetArray[i].size/2 - ball->rad)){
					targetArray[i].drawFilledSquareLCD(LCD_WIDTH,LCD_HEIGHT,FRAME_BUF_1, FRAME_BUF_2,
										    			FRAME_BUF_1_ADDRESS, FRAME_BUF_2_ADDRESS, FRAME_BUF_2,LCD_COLOR_BLACK, &targetArray[i]);
					targetArray[i].isActive=false;
					ball->xSpeed *= -1; //bounce from left:
				}
				if(ball->base.xCoord == (targetArray[i].base.xCoord + targetArray[i].size/2 + ball->rad)){
					targetArray[i].drawFilledSquareLCD(LCD_WIDTH,LCD_HEIGHT,FRAME_BUF_1, FRAME_BUF_2,
										    			FRAME_BUF_1_ADDRESS, FRAME_BUF_2_ADDRESS, FRAME_BUF_2,LCD_COLOR_BLACK, &targetArray[i]);
					targetArray[i].isActive=false;
					ball->xSpeed *= -1; //bounce from right:
				}
		}
		}
	}
...
void move_cball(cBall* ball, uint16_t lcd_width, uint16_t lcd_height) {
    ...
    if (ball->base.xCoord - ball->rad < 0) {
        ball->base.xCoord = ball->rad;
        ball->xSpeed = -ball->xSpeed;
    } else if (ball->base.xCoord + ball->rad > lcd_width) {
        ball->base.xCoord = lcd_width - ball->rad-2;
        ball->xSpeed = -ball->xSpeed;
    }
    if (ball->base.yCoord - ball->rad < 0) {
        ball->base.yCoord = ball->rad;
        ball->ySpeed = -ball->ySpeed;
    } else if (ball->base.yCoord + ball->rad > lcd_height) {
        ball->base.yCoord = lcd_height - ball->rad-2;
        ball->ySpeed = -ball->ySpeed;
    }
...
int main(void)
{
  /* USER CODE BEGIN 1 */
	...
	char str[10];
	uint8_t level;
	cBall ball=*newBall(100,200,10,1,1);

    //targetArray[0]=*newTarget(100,100,20,true);
  ...
  //Test of the LCD
  lcdSimpleTest();
  //targetArray[0].drawFilledSquareLCD(LCD_WIDTH,LCD_HEIGHT,FRAME_BUF_1, FRAME_BUF_2,
      	    			//FRAME_BUF_1_ADDRESS, FRAME_BUF_2_ADDRESS, FRAME_BUF_2,LCD_COLOR_RED, &targetArray[0]);
  HAL_TIM_Base_Start_IT(&htim10);
  timer_val = __HAL_TIM_GET_COUNTER(&htim10);
  timer_val=180;
  level=180;
  move_cball(&ball,LCD_WIDTH, LCD_HEIGHT);
  /* USER CODE END 2 */
  ...
  while (1)
  {
	  ...
	  /*sprintf(str,"%d", timer_val);
	  BSP_LCD_DisplayStringAt(73,3,str,CENTER_MODE);
	  HAL_Delay(5);
	  timer_val--;
	  if(timer_val==0){
		  timer_val=level;
